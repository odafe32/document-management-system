        ...
        <div class="page-header">
            <div class="page-header-content">
                ...
                <div style="color: rgba(255,255,255,0.8); font-size: 0.875rem;">
                    ...
                    Showing feedbacks assigned to you and available unassigned feedbacks
                </div>
            </div>
        </div>
                            ...
                            <div class="feedback-header">
                                <div style="flex: 1;">
                                    ...
                                    <div class="feedback-meta">
                                        <span><i class="fas fa-user"></i> {{ $feedback->user->name ?? 'Unknown User' }}</span>
                                        ...
                                        @if($feedback->admin || $feedback->staff)
                                            <span><i class="fas fa-user-tie"></i> {{ $feedback->admin->name ?? $feedback->staff->name ?? 'Unassigned' }}</span>
                                        @endif
                                ...
                                @if($feedback->has_reply)
                                    <div style="background: #f0f9ff; padding: 1rem; border-radius: 8px; margin-top: 1rem; border-left: 4px solid #3b82f6;">
                                        <h5 style="margin: 0 0 0.5rem 0; color: #1e40af; font-size: 0.875rem;">
                                            <i class="fas fa-reply"></i> Reply by {{ $feedback->admin->name ?? $feedback->staff->name ?? 'Admin' }}
                                        </h5>
                ...
                <div class="sidebar-card">
                    ...
                    <div class="sidebar-content">
                        <div style="font-size: 0.875rem; color: #6b7280; line-height: 1.6;">
                            @php
                                $myResolvedCount = \App\Models\Feedback::where(function($q) {
                                    $q->where('admin_id', Auth::id())->orWhere('staff_id', Auth::id());
                                })->where('status', 'resolved')->count();
                                $myTotalCount = $stats['assigned_to_me'];
                                $responseRate = $myTotalCount > 0 ? round(($myResolvedCount / $myTotalCount) * 100) : 0;
                                
                                $availableCount = \App\Models\Feedback::whereNull('admin_id')->whereNull('staff_id')->count();
                                $myHighPriorityCount = \App\Models\Feedback::where(function($q) {
                                    $q->where('admin_id', Auth::id())->orWhere('staff_id', Auth::id());
                                })->where('priority', 3)->count();
                                $myThisWeekCount = \App\Models\Feedback::where(function($q) {
                                    $q->where('admin_id', Auth::id())->orWhere('staff_id', Auth::id());
                                })->where('created_at', '>=', now()->startOfWeek())->count();
                            @endphp
                            <p><strong>My Response Rate:</strong> {{ $responseRate }}%</p>
                            <p><strong>Available to Assign:</strong> {{ $availableCount }}</p>
                            <p><strong>High Priority (Mine):</strong> {{ $myHighPriorityCount }}</p>
                            <p><strong>This Week (Mine):</strong> {{ $myThisWeekCount }}</p>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    function openFeedbackModal(feedbackId) {
        ...
        fetch(`/admin/feedbacks/${feedbackId}/details`, {
            method: 'GET',
            headers: {
    ...
    function loadFeedbackContent(feedback) {
        ...
        let contentHTML = `
            <div class="feedback-details">
                ...
                <div class="detail-row">
                    ...
                    <span class="detail-value">${feedback.admin.name}</span>
                </div>
                ` : ''}
            </div>
            ...
            <div class="attachment-info">
                ...
                <a href="/admin/feedbacks/${feedback.id}/download" class="btn-secondary" style="margin-left: auto;">
            ...
            <div class="reply-section">
                ...
                <p style="margin: 0; font-size: 0.875rem; color: #6b7280;">
                    Replied by ${feedback.admin ? feedback.admin.name : 'admin'} • ${feedback.replied_at_human}
                </p>
            </div>
            ` : `
        ...
        let actionsHTML = `
            <div class="action-group">
                ${!feedback.admin_id || feedback.admin_id !== '{{ Auth::id() }}' ? `
    ...
    function submitReply(event) {
        ...
        fetch(`/admin/feedbacks/${currentFeedbackId}/reply`, {
            method: 'POST',
            body: formData,
            headers: {
                'X-CSRF-TOKEN': getCSRFToken(),
    ...
    function assignFeedback(event) {
        ...
        fetch(`/admin/feedbacks/${currentFeedbackId}/assign`, {
            method: 'POST',
            body: formData,
            headers: {
                'X-CSRF-TOKEN': getCSRFToken(),
    ...
    function updateStatus(status) {
        ...
        fetch(`/admin/feedbacks/${currentFeedbackId}/status`, {
            method: 'PATCH',
            body: formData,
            headers: {
                'X-CSRF-TOKEN': getCSRFToken(),
