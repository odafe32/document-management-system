        ...
        <div class="page-header">
            <div class="page-header-content">
                <div class="page-title">
                    ...
                    <h1 style="color: #fff">Document Management</h1>
                </div>
                    ...
                    @forelse($documents ?? [] as $document)
                        <div class="document-item">
                            ...
                            <div class="document-actions">
                                <button onclick="viewDocument('{{ $document->id }}', '{{ addslashes($document->title) }}', '{{ addslashes($document->description ?? '') }}', '{{ $document->category }}', '{{ $document->visibility }}', '{{ $document->downloads ?? 0 }}', '{{ $document->created_at->format('Y-m-d') }}', '{{ $document->file_size ?? 'Unknown' }}', '{{ $document->original_filename ?? $document->title }}')" class="btn-info" title="View Document">
                                    <i class="fas fa-eye"></i>
                                </button>
                                <a href="{{ route('staff.documents.download', $document) }}" class="btn-secondary" title="Download">
                                    <i class="fas fa-download"></i>
                                </a>
                                <a href="{{ route('staff.documents.edit', $document) }}" class="btn-success" title="Edit">
            ...
            <div class="sidebar">
                ...
                <div class="sidebar-card">
                    ...
                    <div class="sidebar-content">
                        ...
                        <a href="{{ route('staff.documents') }}" class="btn-secondary" style="width: 100%; justify-content: center;">
...
<script>
    ...
    function viewDocument(documentId, title, description, category, visibility, downloads, createdAt, fileSize, filename) {
        ...
        document.getElementById('viewDownloadBtn').onclick = () => {
            window.location.href = `/staff/documents/${documentId}/download`;
        };
        document.getElementById('viewEditBtn').onclick = () => {
            window.location.href = `/staff/documents/${documentId}/edit`;
        };
    ...
    function confirmDelete(documentId, documentTitle) {
        ...
        document.getElementById('deleteForm').action = `/staff/documents/${documentId}`;
