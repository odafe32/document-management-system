        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            @if($announcement->expiry_date)
                <span style="font-size: 0.875rem; color: #6b7280;">
                    ...
                    Expires on {{ $announcement->expiry_date->format('F j, Y') }}
                </span>
            @endif
        </div>

        <form action="{{ route('staff.announcements.update', $announcement) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                ...
                <div class="card-content">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="body" class="form-control textarea @error('body') border-red-500 @enderror" 
                                  required placeholder="Enter announcement content">{{ old('body', $announcement->body) }}</textarea>
                        @error('body')
                            <p class="text-error">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <select name="category" class="form-control @error('category') border-red-500 @enderror" required>
                                ...
                                <option value="exam" {{ old('category', $announcement->category) == 'exam' ? 'selected' : '' }}>Exam</option>
                                <option value="timetable" {{ old('category', $announcement->category) == 'timetable' ? 'selected' : '' }}>Timetable</option>
                                <option value="memo" {{ old('category', $announcement->category) == 'memo' ? 'selected' : '' }}>Memo</option>
    ...
    <p class="help-text">Leave empty to show announcement to all departments, or select a specific department</p>
                        ...
                        <div class="form-group">
                            ...
                            <select name="visibility" class="form-control @error('visibility') border-red-500 @enderror" required>
                                ...
                                <option value="public" {{ old('visibility', $announcement->visibility) == 'public' ? 'selected' : '' }}>Public (Everyone can see)</option>
                                <option value="staff" {{ old('visibility', $announcement->visibility) == 'staff' ? 'selected' : '' }}>Staff Only</option>
                                <option value="student" {{ old('visibility', $announcement->visibility) == 'student' ? 'selected' : '' }}>Students Only</option>
                            </select>
                            @error('visibility')
                                <p class="text-error">{{ $message }}</p>
                            @enderror
                    ...
                    <p class="help-text">Uncheck this to hide the announcement from public view.</p>
                </div>
            </div>
                ...
                <div class="card-content">
                    ...
                    @if($announcement->attachment)
                        <div class="current-attachment">
                            ...
                            <div class="attachment-info" style="flex: 1;">
                                <h4>{{ $announcement->attachment_filename }}</h4>
                                <div class="attachment-meta">
                                    Current attachment • Uploaded {{ $announcement->created_at->diffForHumans() }}
                                </div>
                            </div>
                            <div style="display: flex; gap: 0.5rem;">
                                <a href="{{ route('staff.announcements.download', $announcement) }}" class="btn-secondary">
                                    <i class="fas fa-download"></i>Download
                                </a>
                                <button type="button" onclick="removeAttachment()" class="btn-danger">
                    ...
                    <div class="form-group">
                        <label class="form-label">
                            ...
                            {{ $announcement->attachment ? 'Replace Attachment' : 'Attach File' }}
                        </label>
                        <p style="font-size: 0.875rem; color: #6b7280; margin-bottom: 1rem;">
                            {{ $announcement->attachment ? 'Upload a new file to replace the current attachment.' : 'You can attach a document, image, or other file to this announcement.' }}
                        </p>
...
<script>
    ...
    uploadArea.addEventListener('drop', function(e) {
        ...
        if (files.length > 0) {
            ...
            document.getElementById('attachmentName').textContent = `New file selected: ${files[0].name}`;
    ...
    function removeAttachment() {
        if (confirm('Are you sure you want to remove the current attachment?')) {
            ...
            // Add a hidden input to indicate attachment removal
            const removeInput = document.createElement('input');
            removeInput.type = 'hidden';
            removeInput.name = 'remove_attachment';
            removeInput.value = '1';
            document.querySelector('form').appendChild(removeInput);
            
            // Update the label text
            document.querySelector('.form-label i + text()').textContent = 'Attach File';
        }
    }
